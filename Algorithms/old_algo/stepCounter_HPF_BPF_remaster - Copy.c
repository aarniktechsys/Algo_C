...
//              Global State → Cadence Validation → Steps (@ 26 Hz)
//===============================================================================
int process_step_block_remaster(SENSOR_DATA_F* accel_data, SENSOR_DATA_F* gyro_data, uint8_t total_samples) {
	...
	for (int i = 0; i < total_samples; ++i) {
		magnitude[i] = sqrtf((float)accel_data[i].x * accel_data[i].x +
		                      (float)accel_data[i].y * accel_data[i].y +
		                      (float)accel_data[i].z * accel_data[i].z);
	...
	// Normal walking: ~5,000-50,000 mg² (0.005-0.05 g²)
	// Vigorous fidgeting/shaking: >100,000 mg² (extreme motion)
	// Only reject if energy is EXTREMELY high (>100,000 mg²)
	if (energy > 100000.0f) {
#if (DEBUG_SAMPLES)
		printf("[ExtremeEnergy] energy=%.0f mg² > 100k - rejecting extreme motion\n", energy);
#endif
	...
	for (int i = 0; i < peak_count_in_block; ++i) {
		...
		// ===== PHASE 11: Hard 250ms refractory period =====
		// Skip peaks that occur within 250ms (~7 samples @ 26 Hz) of the last accepted peak
		// This prevents double-counting a single step
		// At 26 Hz: 250ms = 6.5 samples, use 7 for safety
		if (distance_since_last_peak < 13) { //7
#if (DEBUG_SAMPLES)
			printf("[Refractory] global_idx=%d, last=%d, dist=%d < 7 (skipped)\n",
		...
		// Walking cadence: 50-160 BPM = 0.375-1.2 sec/step
		// At 52 Hz: 0.375 sec = 19.5 samples, 1.2 sec = 62.4 samples
		// STRICT initially (19-62 samples) to confirm walking pattern
		// LENIENT after confirmed (18-65 samples) to allow natural cadence variation
		uint8_t cadence_min = (g_step_state.consecutive_peaks < 3) ? 19 : 18;   // 160-173 BPM strict
		uint8_t cadence_max = (g_step_state.consecutive_peaks < 3) ? 62 : 65;   // 50 BPM strict / 48 BPM lenient
	...
	// At 26 Hz with 48 samples per block: 48 samples = 1846 ms (1.85 seconds)
